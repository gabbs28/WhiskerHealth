...
const singlePrivateFileUpload = async (
    file:
        | PromiseLike<{ originalname: any; mimetype: any; buffer: any }>
        | { originalname: any; mimetype: any; buffer: any },
) => {
    const { originalname, mimetype, buffer } = await file;
    const path = require('path');
    // name of the file in your S3 bucket will be the date in ms plus the extension name
...
const singleMulterUpload = (
    nameOfKey: any, //'image' comes in
) => multer({ storage: storage }).single(nameOfKey);
const multipleMulterUpload = (nameOfKey: any) => multer({ storage: storage }).array(nameOfKey);
